...
- `socMsgListContentsParameters` is of type [SocMsgListContentsParameters](./src/main/java/com/ringcentral/definitions/SocMsgListContentsParameters.java)
- `result` is an empty string
...
- `createContentRequest` is of type [CreateContentRequest](./src/main/java/com/ringcentral/definitions/CreateContentRequest.java)
- `result` is an empty string
...
RestClient rc = new RestClient(clientID, clientSecret, serverURL);
rc.authorize(jwtToken);
String result = rc.cx().socialMessaging().v1().identities().list(listSocialMessagingIdentitiesParameters);
rc.revoke();
```

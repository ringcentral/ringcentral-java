...
RestClient rc = new RestClient(clientID, clientSecret, serverURL);
rc.authorize(username, extension, password);
PerformanceCallsAggregatesResponse result = rc.restapi(apiVersion).account(accountId).analytics().performance().calls().aggregate().post(performanceCallsAggregatesRequest);
rc.revoke();
```

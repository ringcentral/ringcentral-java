package com.ringcentral.definitions;

public class AssignMultipleDevicesAutomaticLocationUpdates {
    /** */
    public String[] enabledDeviceIds;

    public AssignMultipleDevicesAutomaticLocationUpdates enabledDeviceIds(
            String[] enabledDeviceIds) {
        this.enabledDeviceIds = enabledDeviceIds;
        return this;
    }

    /** */
    public String[] disabledDeviceIds;

    public AssignMultipleDevicesAutomaticLocationUpdates disabledDeviceIds(
            String[] disabledDeviceIds) {
        this.disabledDeviceIds = disabledDeviceIds;
        return this;
    }
}

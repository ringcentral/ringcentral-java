...
public class MessageStoreCallerInfoResponseTo {
     ...
     * communicate by means of short internal numbers, for example when calling to other extension
     * or sending/receiving Company Pager messages
     */
    ...
    /**
     * Contains party location (city, state) if one can be determined from a phone number. This
     * property is filled only when `phoneNumber` is not empty and a server can calculate location
     * information from it (for example, this information is unavailable for US toll-free numbers)
     */
    ...
    /**
     * The `true` value specifies that message is sent exactly to this recipient. Returned in the
     * `to` field for group MMS. Useful if one extension has several phone numbers&#039;
     */
    ...
    /**
     * Enum: AllLinesInUse, Undefined, NoFaxSendPermission, NoInternationalPermission, NoFaxMachine,
     * NoAnswer, LineBusy, CallerHungUp, NotEnoughCredits, SentPartially,
     * InternationalCallingDisabled, DestinationCountryDisabled, UnknownCountryCode, NotAccepted,
     * InvalidNumber, CallDeclined, TooManyCallsPerLine, CallFailed, RenderingFailed, TooManyPages,
     * ReturnToDBQueue, NoCallTime, WrongNumber, ProhibitedNumber, InternalError,
     * FaxSendingProhibited, ThePhoneIsBlacklisted, UserNotFound, ConvertError, DBGeneralError,
     * SkypeBillingFailed, AccountSuspended, ProhibitedDestination, InternationalDisabled
     */
     ...
     * 18661234567. But sometimes it could be returned from database with some formatting applied,
     * for example (866)123-4567. This property is filled in all cases where parties communicate by
     * means of global phone numbers, for example when calling to direct numbers or

...
public class ParsePhoneNumberResponseV2Results {
    ...
    /**
     * Contains several items for the `results.category` response&#039;s attribute value
     * `Ambiguous`, where each item represents formatting for a specific category from the list of
     * categories between which ambiguity happened, e.g. `Extension` and `Regular`, or `Extension`
     * and `ShortCode`.
     *
     * <p>Absent if the request&#039;s parameter `resultContent.includeFormats` value is set to
     * `false`.
     */

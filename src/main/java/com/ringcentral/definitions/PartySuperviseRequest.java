...
public class PartySuperviseRequest {
    ...
    /**
     * Specifies if auto-answer SIP header should be sent. If auto-answer is set to `true`, the call
     * is automatically answered by the supervising party, if set to `false` - then the supervising
     * party has to accept or decline the monitored call Default: true
     */
    ...
    /**
     * Specifies session description protocol (SDP) setting. The possible values are
     * &#039;sendOnly&#039; (only sending) meaning one-way audio streaming; and &#039;sendRecv&#039;
     * (sending/receiving) meaning two-way audio streaming Enum: sendOnly, sendRecv
     */

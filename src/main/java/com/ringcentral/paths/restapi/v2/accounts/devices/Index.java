...
public class Index {
     ...
     *
     * <p>- if the value of `keepAssetsInInventory` is `true`, the given device is moved to
     * unassigned devices and the number is moved to the number inventory; - if the value of
     * `keepAssetsInInventory` is `false`, the given device and number is removed from the account;
     * - if the parameter `keepAssetsInInventory` is not set (empty body), default value `true` is
     * set.
     *
     * <p>HTTP Method: delete Endpoint: /restapi/v2/accounts/{accountId}/devices/{deviceId} Rate
     * Limit Group: Medium App Permission: EditAccounts User Permission: EditAccountDevices
     */
    public RemoveLineResponse delete(RemoveLineRequest removeLineRequest)
